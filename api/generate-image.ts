...
export const config = {
  maxDuration: 60, // Set timeout to 60s
};
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    // --- MAPPING MODEL ID ---
    // Mapping string dari frontend ke Model ID Vertex AI yang sebenarnya
    let vertexModelId = 'gemini-1.5-flash-002'; // Default Fallback

    if (modelKey === 'gemini-2.5-flash-image') {
       // Gunakan Gemini 1.5 Flash 002 (Versi optimized terbaru di Vertex) 
       // Atau gunakan 'gemini-2.0-flash-exp' jika tersedia di region Anda
       vertexModelId = 'gemini-1.5-flash-002'; 
    } else if (modelKey === 'gemini-3-pro-image-preview') {
       // Gunakan Gemini 1.5 Pro 002 (Versi quality terbaru di Vertex)
       // Atau gunakan 'gemini-exp-1206' untuk fitur eksperimental terbaru
